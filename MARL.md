...
1. Guy who did RL at OpenAI for 3 years thinks the frontier is simply better, more realistic reward modeling (reward modeling is the training version of evals, basically what are you pointing it at)  
2. Randall take is that this long form thinking is encouraged in RL rather than just sampling the environment (what does this command do? rather than let me reason thru python garbage collection?), which is suboptimal  
3. He says that interrupting the model is taking it out of training distribution. Backed up by the mythos model whitepaper where interruption causes it to perform worse  
4. Including user turns in training data seems to be an important belief that he has  
...
5. He also mentioned transfer amongst RL tasks as being often negative. Definitely not positive consistently.  
6. His overall worry is local incentives of labs causing us to squander intelligence, rather than broad misalignment issues. Labs will do the easiest ROI to sell investors on, which is test time compute scaling on RSI.   
7. RL at that stage was simply   
8. Follow up with him  
...
9. Another take was that a larger risk than civilization level misalignment is local incentives of companies making AI falling suspect to classic big tech slop issues, focusing on benchmaxxing rather than real value  
10. What data strongly assists LLM ability to engage in game theoretical situations like Mafia, Avalon, etc? Thereby boosting EQ by learning theory of mind with engaged parties  
11. Which RL tasks have negative transfer? 
