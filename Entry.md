...
- slate was continual learning. one of the things that bottlenecked it was needing to label the right answers for everything manually. and in cases where even the right label did not exist due to lack of backend infra, needing to build the backend infra, and keep the model up to date with its changing use of tools. do we still have all the slate code?
