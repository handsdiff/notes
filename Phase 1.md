...
- once supported-app data is semantically legible and predictions remain poor, we should move the bottleneck to data quantity, model capacity, or the learning objective
- the reasonable middle ground to actual attention tracking is cursor tracking with AX tree resolution
- possibly useful for the reasoning ablation https://arxiv.org/pdf/2509.06160 reer from bytedance (gwern flagged it)
...
For an eligible stable surface, the collector captures the target window's screen-coordinate rectangle, retains the full source image, and runs local OCR. Current schema-7 collection uses zero fixed preview crop. Offline `ax-pane-read-v2` reconstruction selects an active pane from the bounded Accessibility ancestry captured at the interaction point, reruns OCR over that rectangle, and preserves its method and confidence. When AX cannot prove a pane, the record uses an explicit pointer-local fallback or unresolved disposition rather than pretending to have equivalent identity. Earlier fixed-crop and pre-schema-7 sessions remain immutable legacy evidence. These are deliberately rough interaction-surface proxies, not claims about gaze.
...
The compiler derives the model-facing destination from that same pre-mutation evidence rather than copying every raw label into the query. It may distinguish an editor from an integrated terminal when the focused Accessibility role and title support that distinction, and it omits a background editor title when it would misdescribe the focused terminal. Literal terminal program titles can identify known shells or agent interfaces. Machine- or workspace-specific aliases require an explicit, versioned mapping recorded in the dataset manifest; they are not generalized into universal rules. An arbitrary earlier READ is not used to guess the current WRITE surface. When the evidence cannot distinguish shell from agent, the interaction mode remains unknown while the original conditioning state remains available for audit and later reinterpretation.

The compiler also projects schema-7 READ identity from the same immutable pane evidence used to reconstruct its content. The model-facing source always identifies the application; it names a main-content, text, scroll, semantic-container, or generic active-pane surface only when the contemporaneous AX selection supports that description. A subpane does not inherit an outer window title that may name a background editor or document. OCR text, later WRITEs, and earlier READs are never used to guess a more convenient identity. Pre-schema-7 READs retain their historical application/window serialization exactly. Manifests separately count legacy READs, proper schema-7 AX selections, schema-7 pointer fallbacks, and unresolved schema-7 observations, because raw schema capability is not equivalent to successful pane reconstruction.

Event validity, episode closure, and target eligibility remain separate. Faithful semantic WRITEs remain auditable even when they do not become independent history or loss. The current conservative target set requires a closed evidence-backed persistent-editor episode with at least 40 trimmed authored characters and six authored words. A composition with a proven submission boundary and proven prompt onset may instead receive loss with at least four trimmed authored characters, because the submission supplies objective closure even when the thought is concise. Closed shorter compositions remain model-facing history; unresolved or unclosed fragments receive no loss and do not masquerade as thoughts in later context. Grounded paste actions do not bypass the applicable substantive authored-content threshold. Records explicitly marked ineligible for Phase 1—including future displayed model predictions—are excluded from both targets and Phase 1 context.
...
3. causally compile verified micro-events and inspect the model-facing READ content and WRITE destination;
4. construct and audit closed composition episodes, their causal context, and the actual loss-bearing targets;
...
The current training candidate implements this boundary with the `phase1-semantic-v13` reducer, `phase1-causal-v16` causal micro-event compiler, `phase1-raw-episode-v8` constructor and its `phase1-raw-episode-causal-v8` projection, and the `phase1-token-pack-v7` packer. Semantic v13 reconstructs the active READ surface from retained full-window evidence before applying pane-aware overlap removal. Causal v16 derives compact, evidence-bound model-facing identities for both WRITE destinations and READ sources without replacing their original audit state. Episode v8 uses the logical WRITE destination—rather than volatile raw window or element labels—to group micro-WRITEs, while continuous editable state, affected composition region, novel causally available READs, and submission remain independent boundaries. On validated submitted-prompt surfaces, the episode must begin from an empty or unpopulated prompt field, or at a proven causal partition after the prior same-surface composition: a novel READ, an outside WRITE, or a prior submission. An unexplained nonempty prompt onset is excluded rather than trained as a suffix. These version names identify the present candidate interpretation, not timeless theoretical requirements. Historical Obsidian Git data remains useful for unit tests, but the foundational experiment uses prospective, interleaved browser, Codex, Obsidian, and supported editor/terminal activity.
...
for each chronological block:
    ...
    continue the personalized Qwen checkpoint on only the just-scored block
```
...
The foundational continual baseline presents each chronological example once: it restores the preceding optimizer-state checkpoint and trains only on the newly scored block. Explicit replay is a separately measured intervention for preserving older workflows when forgetting becomes material; it must not be introduced implicitly by repeatedly training old blocks.
...
For foundational update $k$, let $B_k$ be the newly scored chronological block. The personalized Qwen objective is:

$$
\boxed{
\mathcal L_k(\theta)
=
\mathcal L_{\mathrm{BC}}(\theta;B_k)
}.
$$

The candidate restores the preceding personalized checkpoint and optimizer state $\theta_k$, is optimized only on $B_k$ after that block has been scored, is checked for numerical failure, and is stored as $\theta_{k+1}$ together with run diagnostics. It is neither restarted from the base model nor trained again on all preceding blocks. A parameter-efficient adapter is the default foundational implementation because it makes rapid training, versioning, and rollback tractable; the data and objective are unchanged if later experiments use full-model updates. A later prospective harness may add the recent/replay mixture in Section 4.3 without changing which target tokens receive loss. Practical starting points for LoRA learning rate, batch size, rank, scaling, and epoch count are reported by O'Neill et al. [8]; these are sweep priors rather than assumed optima because their study uses static, judge-filtered SFT data rather than continual personal action data.
...
This reconstruction deliberately separates a **human opportunity clock** from a **model serving policy**. The human clock begins immediately after the latest observable material action because reading, deciding, and physically expressing the next thought are all time that a successful suggestion could save. The three-second delay does not mean that thinking begins three seconds later. It is only a provisional debounce on inference, chosen to avoid issuing a model call after every low-level action. Starting the human clock at the delayed sample would mechanically subtract three seconds from every opportunity and confuse a product throttle with a claim about cognition.

A canonical READ or completed prior WRITE can reset the clock because it changes the information or work product available to the person. Focus, pointer movement, application routing, and the target's own mutations ordinarily express or execute an intention rather than introduce new material information, so they do not reset it. The model nevertheless cannot begin before the material context it needs has become available to the pipeline; OCR readiness and human activity therefore remain separate timestamps. This is an observable wall-clock proxy, not mind reading: long gaps can include unobserved inactivity and remain visible as uncertainty rather than being treated as stronger evidence of thought.
...
**Current status: completed for the candidate closed-episode baseline, with per-session auditing continuing.** The excluded viewer, prospective Obsidian/Chrome/Codex/VS Code collector, raw-first semantic reducer, causal micro-event compiler, episode constructor, causal episode projection, Qwen packer, reconstruction/authorship audits, and three-example Qwen mechanical smoke are implemented. The current candidate is `phase1-semantic-v13`, `phase1-causal-v16`, `phase1-raw-episode-v8`, and `phase1-raw-episode-causal-v8`. New authoritative sessions still require sampling for missing events, temporal-ordering error, incorrect content inclusion, authorship error, READ-source and WRITE-destination ambiguity, episode-boundary disagreement, future leakage, unresolved evidence, and target exclusions. A recurrent material failure or a collector/reducer/episode/compiler change reopens this gate and creates a new versioned lineage.
...
4. Implement and audit the deterministic raw-only semantic reducer, currently `phase1-semantic-v13`, with screenshot-derived active-surface READ reconstruction, pane-aware overlap, stable lineage, and explicit unresolved dispositions.
5. Implement and audit the causal micro-event compiler, currently `phase1-causal-v16`, including strict temporal cutoffs, raw-evidence verification, a compact model-facing WRITE destination derived from pre-mutation Accessibility evidence, and a conservative model-facing READ source derived from contemporaneous pane evidence.
6. Implement and audit the raw-authoritative closed-composition constructor and causal projection, currently `phase1-raw-episode-v8` and `phase1-raw-episode-causal-v8`, including logical-destination grouping, evidence-gated merging, novel-READ partitioning, proven prompt onset, normalized model-facing WRITE history, substantiveness, complete micro-WRITE dispositions, compact history serialization, and target/context exclusions.
...
### 8.2 Current: prospective confirmation on the promoted pipeline

The historical 224-example and expanded 450-example corpora remain immutable developmental baselines. They were essential for exposing write reconstruction, episode demarcation, model-facing history, READ-surface, destination-identity, timing, and training-run failures, but their `phase1-semantic-v10` / `phase1-causal-v14` / `phase1-raw-episode-v6` interpretation is not silently rewritten into the promoted candidate. The current task is to confirm the same learning objective on newly collected schema-7 sessions using one-second READ delay, three-second WRITE delay, zero fixed preview crop, retained full-window screenshots and Accessibility ancestry, then reconstructing them through semantic v13, causal v16, and episode v8. Schema-7 coverage will grow prospectively; manifests must keep its proper AX selections, pointer fallbacks, and unresolved observations distinct from legacy data rather than treating all schema-7 records as equivalent.

1. Finish each new session without changing collector rules mid-session and preserve it as an immutable raw artifact.
2. Build and audit the versioned active-surface READ evidence, then reduce, causally compile, assemble, and construct closed episodes through the promoted versions above.
3. Inspect missing events, content inclusion, authorship, destination identity, episode boundaries, temporal order, leakage, unresolved evidence, and the actual model-visible context and targets before packing.
4. Freeze the resulting corpus, timing artifact, context plans, tokenizer pack, model routes, decoding settings, cost/latency accounting, and score-before-update ledger before another comparison.
5. Append chronological training blocks to the preceding personalized checkpoint only after scoring each block. Preserve all predictions and exact model inputs so semantic and real-time rubrics can be revised without rerunning providers.
6. Treat historical reruns as developmental evidence and the first untouched promoted-pipeline stream as the beginning of prospective confirmation.
...
No foundational data-path artifact remains missing for prospective confirmation. The historical provider runs and their artifact audits are complete; the next behavioral result awaits untouched data reconstructed through the promoted pipeline. More homogeneous prospective data is required for confirmation rather than for another reinterpretation of the developmental run. Sample-time routing, live display, daily manifests, and replay remain later requirements rather than blockers for the offline comparison.
