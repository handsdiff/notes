...
4. Implement and audit the deterministic raw-only semantic reducer, currently `phase1-semantic-v10`, with stable lineage and explicit unresolved dispositions.
5. Implement and audit the raw-authoritative closed-composition constructor, currently `phase1-raw-episode-v6`, including evidence-gated merging, novel-READ partitioning, proven prompt onset, normalized model-facing WRITE history, substantiveness, and complete micro-WRITE dispositions.
6. Implement and audit the causal episode projection, currently `phase1-raw-episode-causal-v6`, including strict temporal cutoffs, compact history serialization, content-target eligibility, and target/context exclusions.
...
### 8.2 Current: foundational predictive-capacity test

The deterministic multi-session assembly, raw-authoritative episode corpus, five-block ledger, shared 32K semantic context plans, tokenizer pack, three-arm runners, cost and latency accounting, immutable manifests, and prequential rank-32 Tinker update path are implemented and audited. The frozen developmental corpus contains 224 examples in blocks of `50 / 50 / 50 / 50 / 24`: the first 50 are training-only warm-up and the remaining 174 are scored by all three arms. Qwen generation uses temperature `0.6` and seed `17`. The first provider run used warm-started cumulative rehearsal and therefore weighted blocks 1–4 by four, three, two, and one presentations; it remains a versioned developmental baseline rather than the canonical continual update. The corrected provider run restored each preceding checkpoint and trained only the newly scored block, presenting each of the first 200 examples once. Block 5 was scored without a wasteful terminal update. Personalized micro target-token NLL was `2.967753` versus frozen Qwen's `3.657216`, saving `5,406.117` prequential bits across the 174 prospective examples. The prior cumulative-rehearsal run produced NLL `3.077763` and saved `4,542.220` bits. Both versioned result sets remain preserved for direct comparison.

1. Retain both the cumulative-rehearsal and append-only runs as immutable developmental baselines with their distinct update policies explicit.
2. Report chronological, aggregate, per-application, target-length, and paste-stratified results, then inspect the preserved predictions and exact model-visible contexts for substantive usefulness. A later holistic rubric may be applied without rerunning providers.
3. Treat the result as a developmental rerun over data that informed pipeline design, not fresh prospective evidence. Continue with homogeneous one-second-READ/ten-percent-crop collection and a new chronological score-before-update stream.
