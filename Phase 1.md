...
Validate the three arms against the same frozen event and serialized-text plan. Record every generated completion, per-example Qwen pre-update loss, both macro example-average and micro target-token aggregates, any genuinely comparable frontier-model score exposed by its interface, latency, cost, and per-application projections. Before inspecting model identity or aggregate automatic scores, blindly label each generated completion as usable, directionally correct, or wrong/unhelpful under a frozen rubric. The primary cross-model semantic measures are usable rate and usable-or-directionally-correct rate; exact match, correct prefix, and macro/micro character similarity remain secondary diagnostics. Paired prequential NLL and bits saved remain the controlled primary personalization measures for frozen versus personalized Qwen. Inspect predictions directly for an initial capacity signal only after the blind judgments are frozen. The same trace may be rerun for fast, versioned development; these reruns are not fresh prospective evidence. Historical note edits may remain a separate reconstruction diagnostic, but they do not replace the interleaved test.
