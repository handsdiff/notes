...
3. personalized Qwen3.5-9B-Base, which trains first on the 50-example warm-up block, then scores each of the remaining blocks before training cumulatively on that block and all preceding eligible examples, except that the terminal block receives no post-score update because no later evaluation consumes that checkpoint.
...
The deterministic multi-session assembly, raw-authoritative episode corpus, five-block ledger, shared 32K semantic context plans, tokenizer pack, three-arm runners, cost and latency accounting, immutable manifests, and prequential rank-32 Tinker update path are implemented and audited. The frozen developmental corpus contains 224 examples in blocks of `50 / 50 / 50 / 50 / 24`: the first 50 are training-only warm-up and the remaining 174 are scored by all three arms. Qwen generation uses temperature `0.6` and seed `17`. Four cumulative updates follow blocks 1–4; block 5 is scored without a wasteful terminal update. The current provider plan is executable under the separately authorized `$60` Tinker ceiling and projects `$39.485060` including reserve.
