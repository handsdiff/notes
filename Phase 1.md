...
The deterministic multi-session assembly, raw-authoritative episode corpus, five-block ledger, shared 32K semantic context plans, tokenizer pack, three-arm runners, cost and latency accounting, immutable manifests, and prequential rank-32 Tinker update path are implemented and audited. The frozen developmental corpus contains 224 examples in blocks of `50 / 50 / 50 / 50 / 24`: the first 50 are training-only warm-up and the remaining 174 are scored by all three arms. Qwen generation uses temperature `0.6` and seed `17`. The first provider run used warm-started cumulative rehearsal and therefore weighted blocks 1–4 by four, three, two, and one presentations; it remains a versioned developmental baseline rather than the canonical continual update. The corrected runner restores each preceding checkpoint and trains only the newly scored block, presenting each of the first 200 examples once. Block 5 is scored without a wasteful terminal update.

1. Execute and resume the authorized provider arms only against the frozen corpus, pack, context plans, implementation hashes, and cost ceiling.
2. Audit the resulting per-example predictions, Qwen pre-update NLLs, block/update ordering, lineage, latency, usage, and cost before interpreting performance.
3. Report chronological, aggregate, per-application, target-length, and paste-stratified results, then inspect the preserved predictions and exact model-visible contexts for substantive usefulness. A later holistic rubric may be applied without rerunning providers.
4. Treat the result as a developmental rerun over data that informed pipeline design, not fresh prospective evidence. Continue with homogeneous one-second-READ/ten-percent-crop collection and a new chronological score-before-update stream only if the result justifies it.
