...
**Sliding window versus context retrieval.** At the fixed 32K baseline context budget, compare the trailing 32K-token causal suffix against a context containing the most recent 16K tokens plus 16K tokens retrieved from the earlier causally available history. BM25 uses the serialized recent 16K-token suffix as its query. Because a long event-stream query may be dominated by generic interface language, query preprocessing removes or downweights common interface boilerplate using a fixed rule established before prospective evaluation. Candidate chunks must precede that suffix and must have been available before target-action onset. Top-scoring chunks are packed deterministically to the retrieval budget and then restored to chronological order before the recent suffix. Both conditions use identical checkpoint lineages, targets, behavioral-cloning objectives, replay, optimizer exposure, and daily evaluation order. This tests whether selecting related older events is more predictive than allocating the entire context budget to contiguous recent history. Dense, hybrid, reranked, learned, and LongNAP-style reasoned retrieval are possible later extensions but are outside the initial ablation matrix.
