...
3. personalized Qwen3.5-9B-Base, which scores each block before training on that block and all preceding eligible examples, except that the terminal block receives no post-score update because no later evaluation consumes that checkpoint.
