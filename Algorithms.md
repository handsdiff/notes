...
- i need to review the loss function in [[Paper]], keeping in mind E2E-TTT and AssistanceZero. i dont like how its combining losses vs having a batch updated ref policy from online BC that is used as the baseline for online IPO
...
- what would make this more of a superhuman system? i think the more specific version of this question is: are there preferences that are not expressed in behavior? if there are actions that take work to think of, or i do not think of, that would be more helpful for me, then yes. but that covers sampling. without DPO/IPO loss, if i take a sampled action that i would not have taken earlier, then we do not change the model weights. should they be changed in that scenario? the top K sample already produced the gold standard as far as we know? does it simply help with the existence of hard negatives that can result in improved training?
