...
Temporally interleaved, language-based, read-write action streams from daily computer use is a good place to start. Next action prediction is the best forcing function for maximizing user trust and proving the ability to actually convert that context into something tangible and useful. A model trained in this way would learn how one thinks at a fundamental level; it would be able to simulate how one converts inbound information into outbound action. 
