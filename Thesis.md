...
**For every \$1 spent on tokens reading context, ~\$13 of human time went into producing it.** Businesses pay people, on the order of **\$1T** by next year, to gather information, decide what matters, translate it into prompts, and correct work produced without it. This number is doubling every 2 years.
...
Our belief is that daily computer use produces the raw material for such a model. People read articles and documents, write notes, prompt LLMs, receive messages, inspect model outputs, browse, etc for hours each day. The set of these interactions constitutes a comprehensive overview of their work, and can be cleaned and processedinto a temporally interleaved language-based event stream of inbound read information and outbound write actions. Formal next action prediction is the forcing function for learning this conversion. A system that can predict what someone will write, search, edit, or ask next has learned more than their stated preferences or a summary of their past. It has learned how their current context interacts with the goals organizing their work.
