...
A falling training loss is not sufficient evidence that Phase 1 has learned a useful personalized action policy. Failure can enter through capture, target construction, context selection, base-model capability, adaptation, or continual retention. These weak links require different tests and are not interchangeable explanations for the same negative result. In particular, no optimizer can recover information that was absent before the action, and a more elaborate memory system cannot repair invalid targets.

The initial experiments therefore use the following failure map. An escalation is justified only after the diagnostic in the preceding column supports the corresponding weak link.

| Observed result | Most plausible weak link | Discriminating diagnostic | Response or algorithmic escalation |
|---|---|---|---|
| The event timeline, authorship, copied spans, or action boundaries cannot be reconstructed reproducibly | Capture, provenance, or segmentation | Manually audit frozen examples across sources and days; rerun construction from the same raw snapshot; test for target leakage and ordering violations | Fix the collectors and versioned segmentation rules; exclude or down-weight low-confidence examples. Do not interpret model loss until this gate passes. |
| Correct personal context, including manually selected oracle context, does not improve prediction | The relevant cause was not captured, the target is underdetermined from observable history, or the tested model cannot use the signal | Hold a strong model and target set fixed while adding sources cumulatively and comparing automatic context with oracle-selected prior evidence | Collect missing browser, chat, interruption, and task context; revise action granularity or the prediction target; separately test a stronger base model. If oracle context still provides no gain, adding a more complex learning algorithm is not a remedy. |
| Oracle context helps, but automatic context does not, or additional raw history hurts | Context selection, token budget, or noise rather than absence of signal | On identical targets and a matched token budget, compare recent raw-context prompting, BM25 retrieval over prior events, retrieved General User Model propositions [28], and an induced Just-In-Time objective [29] | Improve retrieval or compression before weight training. Escalate from fixed BM25 to provenance-linked semantic memory or objective induction; test LongNAP-style learned reason–retrieve–predict [13] only if simpler retrieval remains the bottleneck. |
| Loss or structured-field accuracy improves, but gains are explained by formatting, location, vocabulary, or short-range repetition | Shortcut learning rather than prediction of semantic action content | Compare against frequency, current-note continuation, and short-history recurrent baselines [20]; decompose operation and location from content; rank the true continuation against hard negatives matched for file, time, length, and style | Make content-sensitive ranking a publication gate and improve target/context construction. If sampled same-style negatives add information, test demonstration-derived contrast such as DITTO [30], while retaining its stronger assumption that the observed action is preferable to those samples. |
| Raw or retrieved context demonstrably helps, but chronological LoRA behavioral cloning does not | Weight-update data, masking, optimization, sample efficiency, adapter capacity, or an initialization poorly suited to later adaptation | Compare in-context learning, retrieval, and LoRA SFT on the same base model, evidence, targets, and cutoff; vary training volume separately from test-time context | Debug serialization, loss masks, batching, and data scale before rejecting weight-space personalization. Then test DITTO [30] or a LongNAP-style learned retrieval system [13]; use meta-learned or E2E-TTT initialization [22] when repeated ordinary updates are specifically slow or unstable. |
| Phase 1 works initially but becomes stale, overfits a recent project, or forgets sparse workflows | Temporal drift, correlated updates, or the stability–plasticity tradeoff | Evaluate every candidate on both a rolling chronological set and a fixed historical set, with domain and action-family slices | Use time-balanced sampling, recent-plus-stratified replay, calibrated recency weighting, and immutable rollback. If these fail, use the continual escalation paths in Section 4.2.3. |
| Personal prediction improves while instruction following, tool use, reasoning, or other required base capabilities regress | Capability interference from fine-tuning rather than failure to model the person | Maintain named archival capability evaluations alongside next-action likelihood and report the tradeoff rather than only the personalized metric | Add teacher distillation or explicit KL anchoring, synthetic task-preservation or self-study data, parameter isolation, additional adapter capacity, or a retrieval–weight hybrid. The capability to retain must be stated explicitly. |
| Training targets are frequently copied, model-authored, expedient, mistaken, or inconsistent with later judgment | Capture noise, ambiguous authorship, or demonstrator suboptimality | Stratify performance by actor, provenance, boundary confidence, source regime, and any independently available quality signal | Filter or weight uncertain examples and never credit assistant text as independent human generation. If a credible ranking signal exists, test robust or ranked-demonstration methods such as D-REX [16]; otherwise Phase 1 should model realized behavior and defer improvement beyond it to the coactive comparisons of Phase 2. |
| Failures concentrate in incompatible workflows or the low-rank adapter saturates | Parametric interference or insufficient capacity | Compare domain and action-family slices, adapter rank, and joint versus partitioned training while holding data fixed | Use partitioned adapters, parameter isolation, additional capacity, or retrieval–weight hybrids rather than forcing one adapter to average conflicting workflows. |
| Oracle tests establish usable signal, but no current context, memory, or weight-update method exploits it | A genuine algorithmic limitation | Reproduce the failure across manifests, future chronological windows, and at least one capability-appropriate base model | Preserve the dataset and report it as a personal continual-learning benchmark. Do not proceed to Phase 2 on the assumption that a failed Phase 1 prior is adequate. |

The escalation order is therefore capture validity, signal existence, context selection, adaptation, and continual retention. The methods in the final column are conditional ablations, not components that should all be added at once. Demonstration-derived contrast is also not a neutral substitute for likelihood training: it assumes that the observed action should outrank sampled alternatives. Likewise, synthetic task-preservation data can protect a named capability but can change the estimator's training distribution. Both effects must be measured on the same chronological targets used to diagnose the original failure.
